...
class FritzBoxCallMonitorOptionsFlowHandler(OptionsFlowWithReload):
    ...
    def _get_options_schema(self) -> vol.Schema:
        ...
        Every field carries a default, because a config entry created by the
        core integration has only `prefixes` -- if that.
        """
        ...
        return vol.Schema(
            {
                ...
                vol.Optional(
                    ...
                    vol.Coerce(int),
                    vol.Range(min=MIN_HISTORY_LIMIT, max=MAX_HISTORY_LIMIT),
                ),
                vol.Optional(
                    CONF_LOOKUP_PROVIDER,
                    default=options.get(CONF_LOOKUP_PROVIDER, DEFAULT_LOOKUP_PROVIDER),
                ): vol.In([provider.value for provider in LookupProvider]),
                vol.Optional(
                    CONF_TELLOWS_API_KEY,
                    description={"suggested_value": options.get(CONF_TELLOWS_API_KEY)},
                ): str,
            }
        )
    ...
    async def async_step_init(
        ...
        return self.async_create_entry(
            ...
            data={
                ...
                CONF_LOOKUP_PROVIDER: user_input[CONF_LOOKUP_PROVIDER],
                CONF_TELLOWS_API_KEY: (
                    user_input.get(CONF_TELLOWS_API_KEY) or ""
                ).strip(),
            },
        )
